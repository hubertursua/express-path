...
var getRouteMethod = function(controllersPath, controller, method) {
	try {
		var routeController = require(controllersPath + controller);
	}
	catch(err) {
		throw new Error("Could not load controller " + controller + " for routing using the path " + controllersPath + controller + ".");
	}
