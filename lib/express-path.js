...
var expressPath = function (app, routeParam, opt) {
	...
	if(routeMap.isValid(map)) {
		...
		for(var i in map) {
			...
			var httpRequestMethod = mapRow[2] || 'all';
			...
			if(routeInfo.isValidCMPath(cmPath)) {
				...
				var controllerPath = MODULE_PATH + options.controllersPath + routeInfo.getControllerPath(cmPath);
				...
				if(controllerHandler.fileExists(controllerPath)) {
					...
					var methodName = routeInfo.getMethod(cmPath);

					// Check if the method exists.
					if(methodHandler.exists(methodName, controllerInstance)) {
						// If it does, then

						// Retrieve the method.
						var method = methodHandler.get(methodName, controllerInstance);
