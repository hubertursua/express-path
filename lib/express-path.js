...
var expressPath = function (app, routeParam, opt) {
	...
	var options = {
		...
		middlewaresPath: ((typeof opt.middlewaresPath === 'undefined') ? "middlewares" : opt.controllersPath),
		expressRoot:     ((typeof opt.expressRoot === 'undefined')     ? (path.dirname(module.parent.parent.filename) + path.sep) : opt.expressRoot),
	...
	var getModuleMethod = function (parentPath, mmPath) {
		if(routeInfo.isValidMMPath(mmPath)) {
			...
			var modulePath = parentPath + path.sep + routeInfo.getModulePath(mmPath);
	...
	if(typeof routeParam === 'string') {
		routeParam = options.expressRoot + routeParam;
	}
	...
	if(routeMap.isValid(map)) {
		...
		for(var i in map) {
			...
			var method = getModuleMethod(options.expressRoot + options.controllersPath, cmPath);

			// Loop through all the middleware-method paths and retrieve each of the methods
			var middlewares = [];
			mmPaths.forEach(function(mmPath) {
				var middlewareMethod = getModuleMethod(options.expressRoot + options.middlewaresPath, mmPath);
